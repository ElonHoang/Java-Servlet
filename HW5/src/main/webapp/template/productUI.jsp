			...
			<div class="col-md-4 ftco-animate fadeInUp ftco-animated">
				<div class="block-7">
					...
					<div class="text-center p-4">
						...
						<a href="productUI?id=${pro.getId()}" class="btn btn-primary d-block px-2 py-3">Add To Bag</a>
					</div>
				</div>
			</div>		
		</c:forEach>
		...
		<ul class="pagination">
			<c:if test="${currentPage > 1}">
				<li class="page-item"><a class="page-link"
					href="productUI?index=${currentPage - 1}">Previous</a></li>
			</c:if>
			<c:forEach begin="1" end="${endPage}" var="i">
				<li class="${currentPage == i ? "page-item active" : "" }"><a
					class="page-link" href="productUI?index=${i}">${i}</a></li>
			</c:forEach>
			<c:if test="${currentPage < endPage}">
				<li class="page-item"><a class="page-link"
					href="productUI?index=${currentPage + 1}">Next</a></li>
			</c:if>
		</ul>
	</nav>
</body>
